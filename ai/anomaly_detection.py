...
class AnomalyDetector:
    ...
    def check(self, reading: dict) -> list:
        anomalies = []
        now = time.time()
        ...
        if vib > self.rms_max * 0.8:
            ...
            anomalies.append(Anomaly(f"anom-vib-{now}", "vibration", sev, "Vibration exceeding threshold", vib, self.rms_max, now))

        if temp > self.temp_max * 0.8:
            sev = "critical" if temp > self.temp_max else "warning"
            anomalies.append(Anomaly(f"anom-temp-{now}", "temperature", sev, "Temperature approaching limit", temp, self.temp_max, now))

        if curr > self.current_max * 0.85:
            sev = "critical" if curr > self.current_max else "warning"
            anomalies.append(Anomaly(f"anom-curr-{now}", "current", sev, "Current draw high", curr, self.current_max, now))
