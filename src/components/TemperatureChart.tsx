...
/**
 * TemperatureChart — SVG line chart with orange line and gradient area fill.
 * Title "TEMPERATURE TREND". Dark background.
 */
export default function TemperatureChart({ data }: TemperatureChartProps) {
  const { linePath, areaPath, gridY, yLabels, minV, maxV, latest } = useMemo(() => {
    const slice = data.slice(-60);
    if (slice.length === 0) {
      return { linePath: '', areaPath: '', gridY: [], yLabels: [], minV: 0, maxV: 100, latest: null as number | null };
    }
    ...
    for (let s = 0; s <= steps; s++) {
      ...
      yLabels.push({ y: py, label: val.toFixed(0) });
    }

    return { linePath, areaPath, gridY, yLabels, minV, maxV, latest: slice[slice.length - 1].v };
  }, [data]);
  ...
  return (
    <div className="panel chart-bg" style={{ height: HEIGHT, padding: 0, overflow: 'hidden', borderRadius: 0 }}>
      <div className="flex items-center justify-between px-3" style={{ height: 22, borderBottom: '1px solid #1a2540', flexShrink: 0 }}>
        <span className="text-[10px] font-semibold tracking-wider" style={{ color: '#94a3b8' }}>TEMPERATURE TREND</span>
        <span className="text-[9px] val-orange font-semibold">{latest != null ? `${latest.toFixed(1)}°C` : '—'}</span>
      </div>
      <svg width="100%" height={HEIGHT - 22} viewBox={`0 0 ${WIDTH} ${HEIGHT - 22}`} preserveAspectRatio="none" style={{ display: 'block' }}>
        ...
        {/* Area fill */}
        {areaPath && <path d={areaPath} fill="url(#tempAreaGrad)" stroke="none" />}
        {/* Line */}
        {linePath && <path d={linePath} fill="none" stroke="#f97316" strokeWidth="1.4" strokeLinejoin="round" strokeLinecap="round" />}
      </svg>
    </div>
  );
}
