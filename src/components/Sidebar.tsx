  ...
  return (
    <div className="sidebar flex flex-col h-full text-xs select-none">
      ...
      <div
        ...
        style={{ borderBottom: '1px solid #1e2d45', background: 'linear-gradient(180deg,#182030 0%,#111827 100%)' }}
      >
        <div className="flex items-center gap-2">
          <Monitor size={13} className="text-blue-400" />
          <span className="font-semibold text-sm text-slate-200 truncate max-w-28">
          ...
          machines.map((machine) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <div
                    className="status-dot-active rounded-full"
                    style={{ width: 8, height: 8, background: '#22c55e', boxShadow: '0 0 4px #22c55e', flexShrink: 0 }}
                  />
                )}
              </div>
            );
      ...
      <div className="px-3 py-2 space-y-1.5" style={{ borderBottom: '1px solid #1e2d45' }}>
        <div className="flex items-center justify-between px-2 py-1" style={{ background: '#0d1520', border: '1px solid #1e2d45' }}>
          ...
          <span>
            ...
            <span className="val-blue"> {rmsX.toFixed(2)}</span>
            <span className="text-slate-500">g </span>
            <span className="text-slate-400">Y:</span>
            <span className="val-cyan"> {rmsY.toFixed(2)}g</span>
          </span>
        </div>
        <div className="flex items-center justify-between px-2 py-1" style={{ background: '#0d1520', border: '1px solid #1e2d45' }}>
      ...
      <div className="px-3 py-2 flex-1">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className="val-blue">{selectedMachine?.rms_min ?? 0.8} - {selectedMachine?.rms_max ?? 2.2} g</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-400">Temperature:</span>
            <span className="val-green">{selectedMachine?.temp_min ?? 40} - {selectedMachine?.temp_max ?? 80}°c</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-400">Current:</span>
            <span>
              <span className="val-blue">{selectedMachine?.current_min ?? 1.0} </span>
              <span className="text-slate-500">+ </span>
              <span className="val-yellow">{selectedMachine?.current_max ?? 3.5} A</span>
            </span>
          </div>
        </div>

        <button onClick={onSaveSettings} className="w-full py-1.5 mt-3 text-xs font-semibold cursor-pointer"
          style={{ background: 'linear-gradient(180deg,#1a4a1a 0%,#0f2e0f 100%)', border: '1px solid #22c55e', color: '#22c55e' }}>
          Save Settings
        </button>
      </div>
      ...
      <div
        ...
        className="px-3 py-2 flex items-center justify-center gap-1.5 cursor-pointer transition-colors"
        style={{ borderTop: '1px solid #1e2d45' }}
        onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; }}
        onMouseLeave={(e) => { e.currentTarget.style.background = ''; }}
      >
        <PlusCircle size={11} className="text-slate-400" />
        <span className="text-slate-400">Add Machine</span>
      </div>
    </div>
  );
}
